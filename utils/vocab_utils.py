...
def load_embed_txt(embed_file):
    ...
    Returns:
      a dictionary that maps word to vector,
      and the size of embedding dimensions.
    """
    emb_dict = dict()
    emb_size = None
    with codecs.getreader("utf-8")(tf.gfile.GFile(embed_file, 'rb')) as f:
        for line in f:
            ...
            emb_dict[word] = vec
            if emb_size:
                if not emb_size == len(vec):
                    raise ValueError("All embedding size should be same.")
            else:
                emb_size = len(vec)
    return emb_dict, emb_size


def get_dataset_and_vocab(src_file, tgt_file,
                          src_vocab_file,
                          tgt_vocab_file,
                          share_vocab):
    # get dataset
    ...
    return src_dataset, tgt_dataset, src_vocab_table, tgt_vocab_table
