...
def get_iterator(src_dataset,
    ...
    if not isinstance(src_dataset, TextLineDataset):
        raise TypeError(
            "Expected src_dataset to be TextLineDataset, "
            "found ", type(src_dataset).__name__)
    if not isinstance(tgt_dataset, TextLineDataset):
        raise TypeError(
            "Expected tgt_dataset to be TextLineDataset, "
            "found ", type(tgt_dataset).__name__)
    if not isinstance(src_vocab_table, HashTable):
        raise TypeError(
            "Expected src_vocab_table to be HashTable, "
            "found ", type(src_vocab_table).__name__)
    if not isinstance(tgt_vocab_table, HashTable):
        raise TypeError(
            "Expected tgt_vocab_table to be HashTable, "
            "found ", type(tgt_vocab_table).__name__)
    ...
    if num_buckets > 1:
        ...
        def key_func(unused_1, unused_2, unused_3, src_len, tgt_len):
            ...
            # Bucket sentence pairs by the length of their
...
def get_classifcation_iterator(
        src_dataset,
        tgt_dataset,
        src_vocab_table,
        tgt_vocab_table,
    ...
    pad_id = tf.cast(src_vocab_table.lookup(
        tf.constant(padding_token)), tf.int32)
    src_tgt_dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset))
    ...
    # Add in sequence lengths.
    src_tgt_dataset = src_tgt_dataset.map(
        lambda src, tgt: (src, tgt, tf.size(src)),
        num_parallel_calls=num_parallel_calls).prefetch(output_buffer_size)
    ...
    def batching_func(x):
        return x.padded_batch(
            ...
            # later on we will be masking out calculations past the true
            # sequence.
            padding_values=(pad_id, pad_id, 0))
