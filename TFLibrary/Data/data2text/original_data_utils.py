...
# CHANGED
from TFLibrary.Data.data2text.text2num import text2num, NumberException
...
def get_datasets(path="../boxscore-data/rotowire"):
    ...
    for dataset in datasets:
        ...
        for i, entry in enumerate(dataset):
            ...
            append_candidate_rels(entry, summ, all_ents,
                                  prons, players, teams, cities, nugz)
...
def save_full_sent_data(outfile, path="../boxscore-data/rotowire", multilabel_train=False, nonedenom=0):
    ...
    for tup in datasets[0]:
        if multilabel_train:
            append_multilabeled_data(
                tup, trsents, trlens, trentdists, trnumdists, trlabels, vocab, labeldict, max_trlen)
        else:
            append_to_data(tup, trsents, trlens, trentdists,
                           trnumdists, trlabels, vocab, labeldict, max_trlen)
    ...
    if nonedenom > 0:
        ...
        none_idxs = [i for i, labellist in enumerate(trlabels) if labellist[
            0] == labeldict["NONE"]]
        random.shuffle(none_idxs)
        # allow at most 1/(nonedenom+1) of NONE-labeled
        num_to_keep = int(math.floor(
            float(len(trlabels) - len(none_idxs)) / nonedenom))
    ...
    for tup in datasets[1]:
        ...
        append_multilabeled_data(tup, valsents, vallens, valentdists,
                                 valnumdists, vallabels, vocab, labeldict, max_vallen)
    ...
    for tup in datasets[2]:
        #append_to_data(tup, valsents, vallens, valentdists, valnumdists, vallabels, vocab, labeldict, max_len)
        append_multilabeled_data(tup, testsents, testlens, testentdists,
                                 testnumdists, testlabels, vocab, labeldict, max_testlen)
...
def prep_generated_data(genfile, dict_pfx, outfile, path="../boxscore-data/rotowire", test=False):
    ...
    for i, entry in enumerate(valdata):
        summ = gens[i]
        append_candidate_rels(entry, summ, all_ents, prons,
                              players, teams, cities, nugz)
    ...
    h5fi["boxrestartidxs"] = np.array(
        np.array(rel_reset_indices) + 1, dtype=int)  # 1-indexed
    h5fi.close()
                        ...
                        for k, word in enumerate(tokes[ent_start:ent_end]):
                            ...
                            if src_idx is not None:
                                ...
                                if targ_idx is None:
                                    ...
                                    assert rulsrcs[i][src_idx] == word and entry[
                                        "summary"][targ_idx] == word
                                    # src_idx, target_idx
                            ...
                            if idthing:  # home, so look in the home row
                                if entry["home_line"][label] == word:
                                    ...
                                    src_idx = 2 * NUM_PLAYERS * \
                                        (len(bs_keys) - 1) + len(bs_keys) - \
                                        len(ls_keys) + col_idx - 1
                            else:
                                if entry["vis_line"][label] == word:
                                    col_idx = ls_keys.index(label)
                                    src_idx = (2 * NUM_PLAYERS + 1) * (len(bs_keys) - 1) + \
                                        len(bs_keys) - \
                                        len(ls_keys) + col_idx - 1
                            if src_idx is not None:
