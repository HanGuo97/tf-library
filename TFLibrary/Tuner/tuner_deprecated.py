...
class HparamOptimizer(object):
    ...
    def _tune(self):
        ...
        def recursive_loop(gpus,
            ...
            Args:
                gpus: list of list of GPU ID's
                configs: list of remaining Optimizer configurations
                hparams: OrderedDict of hparams from previous optimizer
                pbar: `tqdm.trange()` from previous optimizer

            Returns:
                A single or aggregated observation (Scalar) from optimizer
            ...
            # Each optimizer has its own set of GPU's
            num_parallel = len(gpus) if gpus is not None else None
